...
def initialize_board():
    ...
    """
    global board
    print("Enter the initial state of the board (use 0 for empty, 1 for computer's move, -1 for human's move):")
    valid_input = False

    while not valid_input:
        try:
            user_input = input("Enter board state as a single line of 9 numbers (e.g., 0 1 -1 0 1 0 -1 0 0): ")
            board_values = list(map(int, user_input.split()))
            
            if len(board_values) != 9:
                raise ValueError("You must enter exactly 9 values.")
            
            # Convert the flat list into a 3x3 board
            board = [board_values[0:3], board_values[3:6], board_values[6:9]]
            ...
            print("Invalid input. Please enter exactly 9 integers separated by spaces.")
